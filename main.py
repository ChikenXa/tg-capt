...
async def cleanup_old_messages(application):
    """Удаляем старые сообщения бота в 6:00 утра"""
    try:
        current_time = get_moscow_time()
        logger.info(f"🧹 Проверка очистки: {current_time.strftime('%H:%M')} МСК")
        
        if current_time.hour == 6 and current_time.minute == 0:
            logger.info("✅ Начало очистки старых сообщений...")
            ...
            for chat_id in unique_chats:
                try:
                    ...
                    logger.info(f"✅ Очистка завершена в чате {chat_id}: удалено {deleted_count} сообщений, {len(events_to_remove)} каптов")
                    
                except Exception as e:
                    logger.error(f"❌ Ошибка очистки в чате {chat_id}: {e}")
            
            logger.info("✅ Очистка сообщений завершена во всех чатах")
        else:
